...
protected:
    ...
    virtual void drawLabelMaybeCached(QCPPainter *painter, const QFont &font, const QColor &color,
                                      ...
                                      const QString &text);
    virtual QByteArray generateLabelParameterHash() const;  // TODO: get rid of this in favor of invalidation
                                                            // flag upon setters?
...
void QCPDataContainer<DataType>::add(const DataType &data) {
    if (isEmpty() ||
        !qcpLessThanSortKey<DataType>(data, *(constEnd() - 1)))  // quickly handle appends if new data key
...
void QCPDataContainer<DataType>::preallocateGrow(int minimumPreallocSize) {
    ...
    newPreallocSize +=
        (1u << qBound(4, mPreallocIteration + 4, 15)) - 12;  // do 4 up to 32768-12 preallocation, doubling in
...
PlottableType *QCustomPlot::plottableAt(const QPointF &pos, bool onlySelectable, int *dataIndex) const {
    ...
    foreach (QCPAbstractPlottable *plottable, mPlottables) {
        ...
        if (!currentPlottable ||
            (onlySelectable && !currentPlottable->selectable()))  // we could have also passed onlySelectable
                                                                  // to the selectTest function, but checking
                                                                  // here is faster, because we have access to
                                                                  // QCPAbstractPlottable::selectable
            continue;
        if (currentPlottable->clipRect().contains(pos.toPoint()))  // only consider clicks where the plottable
                                                                   // is actually visible
        {
            QVariant details;
...
ItemType *QCustomPlot::itemAt(const QPointF &pos, bool onlySelectable) const {
    ...
    foreach (QCPAbstractItem *item, mItems) {
        ...
        if (!currentItem ||
            (onlySelectable && !currentItem->selectable()))  // we could have also passed onlySelectable to
...
public:
    ...
    */
    ...
    virtual double selectTest(
        ...
        QVariant *details = 0) const;  // actually introduced in QCPLayerable as non-pure, but we want to
